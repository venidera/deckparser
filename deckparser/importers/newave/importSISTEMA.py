...
def importSISTEMA(fdata, dger):
    ...
    if strsearch['line']:
        ...
        while fdata[line][0:4].strip() != '999':
            ...
            if SISTEMA['sss'][str(ssis)]['fict'] == 0:
                ...
                SISTEMA['deficit'][ssis] = {
                    'defpat1': defpat1, 'defpat2': defpat2, 'defpat3': defpat3,
                    'defpat4': defpat4, 'pucortepat1': pucortepat1,
                    'pucortepat2': pucortepat2, 'pucortepat3': pucortepat3,
                    'pucortepat4': pucortepat4}
            line += 1
    ...
    if strsearch['line']:
        ...
        for line in range(linerefini, linereffim):
            ...
            if (fdata[line].strip() != '' and
                    int(fdata[line].split()[0]) < 1900):
                IORI = fdata[line].split()[0]
                IDES = fdata[line].split()[1]
                limites = list()
                limites = [0 for i in range(dger['ni'])]
            # elif fdata[line][0:4].strip() != '' and
            # int(fdata[line][0:4].strip()) in dger['yph']:
            elif (fdata[line][0:4].strip() != '' and
                  int(fdata[line].split()[0]) in dger['yph']):
                ...
                if yearsload == dger['nyears']:
                    SISTEMA['liminter'].append(
                        {'ORI': IORI, 'DES': IDES, 'limites': limites})
                    vtemp = IORI
                    IORI = IDES
                    IDES = vtemp
                    limites = list()
    ...
    if strsfim['line']:
        ...
        for line in range(linerefini, linereffim):
            if fdata[line][0:2] == '  ' and fdata[line].strip() != '':
                ...
                pre = list()
                pre = [0 for i in range(1, 13)]
                pos = list()
                pos = [0 for i in range(1, 13)]
            elif fdata[line][0:3].strip() == 'PRE':
                dados = fdata[line][7:]
                for n in range(0, 12):
                    prefim = ((n+1)*8)
                    preinimov = prefim-8
                    pre[n] = float(dados[preinimov:prefim].strip())
                ...
                SISTEMA['mercado'][SUBSIS] = {
                    'demanda': demanda, 'pre': pre, 'pos': pos}
